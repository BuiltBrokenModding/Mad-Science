...
public class CryotubeBlockGhost extends Block
{
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        else if (!player.isSneaking())
        {
            ...
            CryotubeEntity tileentityfurnace = null;

            // Hook the tile-entity now that we have an idea where it is.
            switch (meta)
            {
            case 1:
                tileentityfurnace = (CryotubeEntity) world.getBlockTileEntity(x, y - 1, z);
                break;
            case 2:
                tileentityfurnace = (CryotubeEntity) world.getBlockTileEntity(x, y - 2, z);
                break;
            }
