...
public abstract class TileEntityInventoryPrefab extends TileEntityRedstonePrefab implements ISidedInventory
{
    /**
    ...
    public TileEntityInventoryPrefab()
    {
        super();
    }

    TileEntityInventoryPrefab(TileEntityFactoryProduct registeredMachine)
    {
    ...
    {
        return this.isItemValidForSlot( slot,
                                        item );
    }
    ...
    public int getInputSlotCount()
    {
        ...
        int inputSlots = 0;
        for (ContainerTemplate slotContainer : containerSlots)
        {
     ...
     */
    public int getSlotCountByType(SlotContainerTypeEnum slotType)
    {
        // Grab a list of all container values from enumeration.
        ContainerTemplate[] containerSlots = this.getRegisteredMachine().getContainerTemplate();
        int i = containerSlots.length;
     ...
     */
    public ItemStack getRecipeResult(SlotContainerTypeEnum... searchPatternArray)
    {
        // Grab the recipe archive object array, exit if we return nothing to compare against.
        RecipeArchive[] recipeArchiveArray = this.getRegisteredMachine().getRecipeArchive();
        ...
        SlotContainerTypeEnum machineOutputSlot = null;

        // Determine slot type, adding item to appropriate list.
        for (SlotContainerTypeEnum slotType : searchPatternArray)
        {
        ...
        for (RecipeArchive singleRecipe : recipeArchiveArray)
        {
            RecipeComponent[] inputRecipeComponents = singleRecipe.getInputIngredientsArray();
            RecipeComponent[] outputRecipeComponents = singleRecipe.getOutputResultsArray();
            ...
            for (RecipeComponent inputComponent : inputRecipeComponents)
            {
            ...
            boolean inputArrayMatch = doArraysMatch( compareInput,
                                                     compareMachine );

            // If input array matches then we will return the output.
            if (inputArrayMatch)
            {
                for (RecipeComponent outputComponent : outputRecipeComponents)
                {
     ...
     */
    public ItemStack getStackInSlotByType(SlotContainerTypeEnum slotType)
    {
        // Check if we have a registered machine present to reference slot types from.
        TileEntityFactoryProduct currentMachine = this.getRegisteredMachine();
        ...
        for (ContainerTemplate currentSlot : currentMachine.getContainerTemplate())
        {
     ...
     */
    public SlotContainerTypeEnum getSlotTypeBySlotID(int slotID)
    {
        TileEntityFactoryProduct currentMachine = this.getRegisteredMachine();
        if (currentMachine == null)
        {
     ...
     */
    public int getSlotIDByType(SlotContainerTypeEnum slotType)
    {
        TileEntityFactoryProduct currentMachine = this.getRegisteredMachine();
        if (currentMachine == null)
        {
    ...
    {
        return this.worldObj.getBlockTileEntity( this.xCoord,
                                                 this.yCoord,
                                                 this.zCoord ) != this
               ? false
     ...
     */
    public void setInventorySlotContentsByType(SlotContainerTypeEnum slotType, ItemStack item)
    {
        TileEntityFactoryProduct currentMachine = this.getRegisteredMachine();
        if (currentMachine == null)
        {
        ...
        for (ContainerTemplate currentSlot : currentMachine.getContainerTemplate())
        {
            if (currentSlot.getSlotType().equals( slotType ))
            {
                ...
                this.setInventorySlotContents( currentSlot.slot(),
                                               item );
            }
        }
    }
