...
public class TileTemplateBlock extends BlockContainer
{
    private TileEntityFactoryProduct registeredProduct;
    ...
    public TileTemplateBlock(TileEntityFactoryProduct registeredMachine)
    {
        // Create Minecraft/Forge to create a block with our specifications.
        super( registeredMachine.getBlockID(),
               Material.iron );
        ...
        BoundingBox loadedBlockBounds = registeredMachine.getBlockBounds();
        if (loadedBlockBounds != null)
        {
            this.setBlockBounds( loadedBlockBounds.getLowerBounds().getModelTranslateX(),
                                 // LOWER BOUNDS
                                 loadedBlockBounds.getLowerBounds().getModelTranslateY(),
                                 loadedBlockBounds.getLowerBounds().getModelTranslateZ(),
                                 loadedBlockBounds.getUpperBounds().getModelTranslateX(),
                                 // UPPER BOUNDS
        ...
        else
        {
            ...
            BoundingBox defaultBlockBounds = registeredMachine.getBlockBounds();
            this.setBlockBounds( defaultBlockBounds.getLowerBounds().getModelTranslateX(),
                                 // LOWER BOUNDS
                                 defaultBlockBounds.getLowerBounds().getModelTranslateY(),
                                 defaultBlockBounds.getLowerBounds().getModelTranslateZ(),
                                 defaultBlockBounds.getUpperBounds().getModelTranslateX(),
                                 // UPPER BOUNDS
    ...
    public TileTemplateBlock(int blockID, Material blockMaterial) // NO_UCD (unused code)
    {
        super( blockID,
               blockMaterial );
    }
     ...
     */
    @Override
    public void addCollisionBoxesToList(World par1World,
                                        int par2,
                                        int par3,
                                        int par4,
                                        ...
                                        Entity par7Entity)
    {
        super.addCollisionBoxesToList( par1World,
                                       par2,
                                       par3,
                                       par4,
                                       par5AxisAlignedBB,
    ...
    public void breakBlock(World par1World, int par2, int par3, int par4, int par5, int par6)
    {
        ...
        TileEntity vanillaTileInstance = par1World.getBlockTileEntity( par2,
                                                                       par3,
                                                                       par4 );
        TileEntityPrefab tileEntityInstance = null;

        // Attempt to cast this block as one of ours.
        if (vanillaTileInstance != null && vanillaTileInstance instanceof TileEntityPrefab)
        {
            tileEntityInstance = (TileEntityPrefab) vanillaTileInstance;

            // Check if we need to play a sound for being destroyed.
            if (this.registeredProduct != null)
            {
                this.registeredProduct.playTriggerSound( SoundTriggerEnum.DESTROYED,
                                                         par2,
                                                         par3,
                                                         par4,
                                                         par1World );
    ...
    {
        return Container.calcRedstoneFromInventory( (IInventory) par1World.getBlockTileEntity( par2,
                                                                                               par3,
                                                                                               par4 ) );
    }
                                    ...
                                    float par9)
    {
        ...
        else if (! player.isSneaking())
        {
            ...
            TileEntity vanillaTileInstance = world.getBlockTileEntity( X,
                                                                       Y,
                                                                       Z );
            TileEntityPrefab tileEntityInstance = null;
            ...
            if (tileEntityInstance != null)
            {
                ...
                tileEntityInstance.onBlockRightClick( world,
                                                      X,
                                                      Y,
                                                      Z,
                                                      player );
    ...
    public void onBlockClicked(World world, int X, int Y, int Z, EntityPlayer player)
    {
        ...
        else if (! player.isSneaking())
        {
            ...
            if (tileEntityInstance != null)
            {
                tileEntityInstance.onBlockLeftClick( world,
                                                     X,
                                                     Y,
                                                     Z,
                                                     player );
    ...
    {
        super.onBlockAdded( world,
                            x,
                            y,
                            z );
        this.setDefaultDirection( world,
                                  x,
                                  y,
                                  z );
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase living, ItemStack stack)
    {
        ...
        if (vanillaTileInstance != null && vanillaTileInstance instanceof TileEntityPrefab)
        {
            ...
            if (tileEntityInstance != null)
            {
                if (this.registeredProduct != null)
                {
                    this.registeredProduct.playTriggerSound( SoundTriggerEnum.CREATED,
                                                             x,
                                                             y,
                                                             z,
                                                             world );
    ...
    {
        this.blockIcon = icon.registerIcon( ModMetadata.ID + ":" +
                                            registeredProduct.getMachineName() );
    }
    ...
    private void setDefaultDirection(World par1World, int par2, int par3, int par4)
    {
        ...
        {
            int l = par1World.getBlockId( par2,
                                          par3,
                                          par4 - 1 );
            int i1 = par1World.getBlockId( par2,
                                           par3,
                                           par4 + 1 );
            int j1 = par1World.getBlockId( par2 - 1,
                                           par3,
                                           par4 );
            int k1 = par1World.getBlockId( par2 + 1,
                                           par3,
                                           par4 );
            byte b0 = 3;
