...
import madscience.button.MadGUIButton;
import madscience.button.MadGUIButtonClickActionEnum;
import madscience.button.MadGUIButtonInvisibleControl;
import madscience.button.MadGUIButtonTypeEnum;
import madscience.container.MadSlotContainer;
import madscience.control.MadGUIControl;
import madscience.control.MadGUIControlTypeEnum;
...
public class MadTileTemplateGUI extends GuiContainer
{
    ...
    {
        super( new MadTileTemplateContainer( entityPlayer,
                                             tileEntity ) );
        this.ENTITY = tileEntity;

        // Query machine registry for GUI control information.
        MadTileEntityFactoryProduct MACHINE =
                MadTileEntityFactory.instance().getMachineInfo( this.ENTITY.getMachineInternalName() );
        ...
        this.TEXTURE = new ResourceLocation( MadModMetadata.ID,
                                             "textures/gui/" +
    ...
    public void actionPerformed(GuiButton button)
    {
        ...
        for (int i = 0; i < BUTTONS.length; i++)
        {
            ...
            {
                // Open Link
                if (guiButton.getClickAction().equals( MadGUIButtonClickActionEnum.OpenLink ) &&
                    Desktop.isDesktopSupported())
                {
                    ...
                    catch (Exception err)
                    {
                        ...
                        helpLink = "";
                        MadModLoader.log().info( "[MadGUIButtonInterface]Unable to cast userData from action " +
        ...
        for (int i = 0; i < this.GUICONTROLS.length; i++)
        {
                ...
                guiControl.getControlType().equals( MadGUIControlTypeEnum.PowerLevelY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          ...
                                          mouseY ))
                {
                    String powerLevelLiteral = String.valueOf( this.ENTITY.getEnergy( ForgeDirection.UNKNOWN ) ) +
                                               "/" +
                                               String.valueOf( this.ENTITY.getEnergyCapacity( ForgeDirection.UNKNOWN ) );
            ...
            if (guiControl.getControlType().equals( MadGUIControlTypeEnum.DamageLevelX ) ||
                guiControl.getControlType().equals( MadGUIControlTypeEnum.DamageLevelY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          guiControl.screenY(),
                                          guiControl.sizeX(),
            ...
            if (guiControl.getControlType().equals( MadGUIControlTypeEnum.HeatGaugeX ) ||
                guiControl.getControlType().equals( MadGUIControlTypeEnum.HeatGaugeY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          guiControl.screenY(),
                                          guiControl.sizeX(),
    ...
    public void initGui()
    {
        ...
        for (int i = 0; i < BUTTONS.length; i++)
        {
            ...
            if (guiButton.getButtonType().equals( MadGUIButtonTypeEnum.InvisibleButton ))
            {
                buttonList.add( new MadGUIButtonInvisibleControl( guiButton.buttonID(),
                                                                  posX + 81,
                                                                  posY - 76,
                                                                  guiButton.sizeX(),
                                                                  guiButton.sizeY() ) );
