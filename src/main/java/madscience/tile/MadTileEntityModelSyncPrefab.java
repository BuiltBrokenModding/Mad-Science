...
class MadTileEntityModelSyncPrefab extends MadTileEntityDamagePrefab
{
    ...
    public void setWorldRenderInformation(MadModelPosition modelPosition, MadModelScale modelScale)
    {
        ...
        MadModelPosition modelWorldPosition = new MadModelPosition( modelPosition.getModelTranslateX(),
                                                                    modelPosition.getModelTranslateY(),
                                                                    modelPosition.getModelTranslateZ() );

        // Create new world renderer scale.
        MadModelScale modelWorldScale = new MadModelScale( modelScale.getModelScaleX(),
                                                           modelScale.getModelScaleY(),
                                                           modelScale.getModelScaleZ() );

        // Create new world render object from the above two components.
        MadModelWorldRender updatedWorldRender = new MadModelWorldRender( modelWorldPosition,
                                                                          modelWorldScale );
    ...
    public void setModelWorldRenderVisibilityByName(String modelName, boolean visible)
    {
        ...
        for (MadModelData modelData : this.entityModelReference)
        {
            if (modelData.getModelPieceName().equals( modelName ))
            {
                ...
                {
                    this.entityModelReference[x] = new MadModelData( visible,
                                                                     modelName );
                    break;
                }
                break;
    ...
    public void readFromNBT(NBTTagCompound nbt)
    {
        ...
        for (int i = 0; i < modelTagList.tagCount(); ++ i)
        {
            ...
            if (partNumber >= 0 && partNumber < this.entityModelReference.length)
            {
                this.setModelDataPartContents( partNumber,
                                               MadModelData.loadModelDataFromNBT( modelPart ) );
            }
        }
        ...
        try
        {
            ...
            MadModelPosition modelWorldPosition = new MadModelPosition( renderWorldPosX,
                                                                        renderWorldPosY,
                                                                        renderWorldPosZ );
            MadModelScale modelWorldScale = new MadModelScale( renderWorldScaleX,
                                                               renderWorldScaleY,
                                                               renderWorldScaleZ );

            // Create new world rendering information from NBT data.
            MadModelWorldRender worldRenderFromNBT = new MadModelWorldRender( modelWorldPosition,
                                                                              modelWorldScale );
        ...
        catch (Exception err)
        {
            ...
            MadModLoader.log().info( "[" + this.getMachineInternalName() +
