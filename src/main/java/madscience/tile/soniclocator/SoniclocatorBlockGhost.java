...
public class SoniclocatorBlockGhost extends Block
{
    public SoniclocatorBlockGhost(int blockID)
    {
        // 'Ghost' block for Soniclocator that serves as physics collision and proxy to GUI.
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        else if (!player.isSneaking())
        {
            ...
            SoniclocatorEntity tileentityfurnace = null;

            // Hook the tile-entity now that we have an idea where it is.
            switch (meta)
            {
            case 1:
                tileentityfurnace = (SoniclocatorEntity) world.getBlockTileEntity(x, y - 1, z);
                break;
            case 2:
                tileentityfurnace = (SoniclocatorEntity) world.getBlockTileEntity(x, y - 2, z);
                break;
            }
            ...
            if (tileentityfurnace != null)
            {
                ...
                {
                case 1:
                    player.openGui(MadForgeMod.instance, MadFurnaces.SONICLOCATOR_TILEENTITY.blockID, world, x, y - 1, z);
                    break;
                case 2:
                    player.openGui(MadForgeMod.instance, MadFurnaces.SONICLOCATOR_TILEENTITY.blockID, world, x, y - 2, z);
                    break;
                }
            }
            return true;
    ...
    public void registerIcons(IconRegister icon)
    {
        ...
        this.blockIcon = icon.registerIcon(MadMod.ID + ":" + MadFurnaces.SONICLOCATOR_INTERNALNAME);
    }
