...
public class CnCMachineBlockGhost extends Block
{
    public CnCMachineBlockGhost(int blockID)
    {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        else if (!player.isSneaking())
        {
            ...
            CnCMachineEntity tileentityfurnace = null;

            // Hook the tile-entity now that we have an idea where it is.
            switch (meta)
            {
            case 1:
                tileentityfurnace = (CnCMachineEntity) world.getBlockTileEntity(x, y - 1, z);
                break;
            }
            ...
            if (tileentityfurnace != null)
            {
                ...
                {
                case 1:
                    player.openGui(MadScience.instance, MadMachines.CNCMACHINE_TILEENTITY.blockID, world, x, y - 1, z);
                    break;
                }
            }
            return true;
