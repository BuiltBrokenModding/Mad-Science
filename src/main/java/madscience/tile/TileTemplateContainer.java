...
import madscience.slot.InputSlot;
import madscience.slot.OutputSlot;
...
public class TileTemplateContainer extends Container
{
    ...
    public TileTemplateContainer()
    {
        super();
    }

    public TileTemplateContainer(InventoryPlayer entityPlayer, TileEntityPrefab tileEntity)
    {
        ...
        for (int i = 0; i < CONTAINERS.length; i++)
        {
            ...
            if (slotContainer.getSlotType().name().toLowerCase().contains( "input" ))
            {
                ...
                this.addSlotToContainer( new InputSlot( tileEntity,
                                                        slotContainer.slot(),
                                                        slotContainer.offsetX(),
                                                        slotContainer.offsetY(),
                                                        slotContainer.getSlotType() ) );
            }
            else if (slotContainer.getSlotType().name().toLowerCase().contains( "output" ))
            {
                ...
                this.addSlotToContainer( new OutputSlot( entityPlayer.player,
                                                         tileEntity,
                                                         slotContainer.slot(),
                                                         slotContainer.offsetX(),
                                                         slotContainer.offsetY(),
        ...
        for (i = 0; i < 3; ++ i)
        {
            for (int j = 0; j < 9; ++ j)
            {
                this.addSlotToContainer( new Slot( entityPlayer,
                                                   j + i * 9 + 9,
                                                   8 + j * 18,
                                                   84 + i * 18 ) );
            }
    ...
    public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotNumber)
    {
        ...
        if (containerSlot != null && containerSlot.getHasStack())
        {
            ...
            if (slotNumber < INV_START + 1)
            {
                ...
                if (! this.mergeItemStack( itemBeingShiftClicked,
                                           INV_START,
                                           HOTBAR_END + 1,
                                           true ))
                {
                ...
                containerSlot.onSlotChange( itemBeingShiftClicked,
                                            copyOfContainerItemStack );
            }
            ...
            else
            {
                ...
                if (this.ENTITY.isItemUsedInInputRecipes( itemBeingShiftClicked ))
                {
                    ...
                    {
                        if (this.ENTITY.isItemValidForSlot( x,
                                                            itemBeingShiftClicked ))
                        {
                            if (! this.mergeItemStack( itemBeingShiftClicked,
                                                       x,
                                                       ACTIVE_SLOT,
            ...
            containerSlot.onPickupFromSlot( entityPlayer,
                                            itemBeingShiftClicked );
        }
