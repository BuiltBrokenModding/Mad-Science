...
import madscience.button.GUIButtonClickActionEnum;
import madscience.button.GUIButtonInvisibleControl;
import madscience.button.GUIButtonTemplate;
import madscience.button.GUIButtonTypeEnum;
import madscience.container.ContainerTemplate;
import madscience.control.GUIControlTypeEnum;
...
public class TileTemplateGUI extends GuiContainer
{
    private TileEntityPrefab ENTITY;
    private GUIControlsTemplate[] GUICONTROLS;
    private ContainerTemplate[] CONTAINERS;
    ...
    public TileTemplateGUI(Container container)
    {
        super( container );
    }

    public TileTemplateGUI(InventoryPlayer entityPlayer, TileEntityPrefab tileEntity)
    {
        super( new TileTemplateContainer( entityPlayer,
                                          tileEntity ) );
        this.ENTITY = tileEntity;

        // Query machine registry for GUI control information.
        TileEntityFactoryProduct MACHINE =
                TileEntityFactory.instance().getMachineInfo( this.ENTITY.getMachineInternalName() );
        ...
        this.TEXTURE = new ResourceLocation( ModMetadata.ID,
                                             "textures/gui/" +
                                             ENTITY.getMachineInternalName() +
                                             ".png" );
    }
    ...
    public void actionPerformed(GuiButton button)
    {
        ...
        {
            GUIButtonTemplate guiButton = BUTTONS[i];

            // Determine if button clicked matches any from our enumeration.
            if (button.id == guiButton.buttonID())
            {
                // Open Link
                if (guiButton.getClickAction().equals( GUIButtonClickActionEnum.OpenLink ) &&
                    Desktop.isDesktopSupported())
                {
                    ...
                    catch (Exception err)
                    {
                        ...
                        helpLink = "";
                        ModLoader.log().info( "[MadGUIButtonInterface]Unable to cast userData from action " +
                                              guiButton.getClickAction() +
                                              " to string!" );
                    ...
                    if (helpLink != null && ! helpLink.isEmpty())
                    {
                        if (GuiScreen.isCtrlKeyDown() && GuiScreen.isShiftKeyDown())
                        {
                            ...
                            {
                                ModLoader.log().info( "Unable to open sandra youtube easter egg link in default browser." );
                            }
                        }
                        else
                        {
                            ...
                            {
                                ModLoader.log().info( "Unable to open wiki link in default browser." );
                            }
                        }
                    }
                }
    ...
    private void displayGauge(int screenX, int screenY, GUIControlsTemplate control, int percentLeft)
    {
        ...
        while (true)
        {
            ...
            drawTexturedModelRectFromIcon( screenX + control.screenX(),
                                           screenY + control.screenY() +
                                           control.sizeY() - x - start,
                                           liquidIcon,
                                           control.sizeX(),
                                           control.sizeX() - (control.sizeX() - x) );
        ...
        drawTexturedModalRect( screenX + control.screenX(),
                               screenY + control.screenY(),
                               control.fillerX(),
                               control.fillerY(),
                               control.sizeX(),
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
    {
        ...
        this.drawTexturedModalRect( this.screenX,
                                    this.screenY,
                                    0,
                                    0,
                                    this.xSize,
        ...
        {
            GUIControlsTemplate guiControl = this.GUICONTROLS[i];

            // -------------
            // POWER LEVEL X
            // -------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.PowerLevelX ))
            {
            ...
            // POWER LEVEL Y
            // -------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.PowerLevelY ))
            {
                int energyLevelY = this.ENTITY.getPowerRemainingScaled( guiControl.sizeY() );
                this.drawTexturedModalRect( screenX + guiControl.screenX(),
                                            screenY + guiControl.screenY() +
                                            guiControl.sizeY() - energyLevelY,
                                            guiControl.fillerX(),
            ...
            // COOKING PROGRESS X
            // ------------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.CookingProgressX ))
            {
            ...
            // COOKING PROGRESS Y
            // ------------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.CookingProgressY ))
            {
                int cookingProgressY = this.ENTITY.getItemCookTimeScaled( guiControl.sizeY() );
                this.drawTexturedModalRect( screenX + guiControl.screenX(),
                                            screenY + guiControl.screenY() +
                                            guiControl.sizeY() - cookingProgressY,
                                            guiControl.fillerX(),
            ...
            // DAMAGE LEVEL X
            // --------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.DamageLevelX ))
            {
            ...
            // DAMAGE LEVEL Y
            // --------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.DamageLevelY ))
            {
                int damageLevelY = this.ENTITY.getDamageValueScaled( guiControl.sizeY() );
                this.drawTexturedModalRect( screenX + guiControl.screenX(),
                                            screenY + guiControl.screenY() +
                                            guiControl.sizeY() - damageLevelY,
                                            guiControl.fillerX(),
            ...
            // HEAT LEVEL X
            // ------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.HeatGaugeX ))
            {
                int heatLevelX = this.ENTITY.getHeatLevelTimeScaled( guiControl.sizeX() );
                drawTexturedModalRect( screenX + guiControl.screenX(),
                                       screenY + guiControl.screenY(),
                                       guiControl.fillerX(),
                                       guiControl.fillerY(),
                                       heatLevelX,
            ...
            // HEAT LEVEL Y
            // ------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.HeatGaugeY ))
            {
                int heatLevelY = this.ENTITY.getHeatLevelTimeScaled( guiControl.sizeY() );
                drawTexturedModalRect( screenX + guiControl.screenX(),
                                       screenY + guiControl.screenY() +
                                       guiControl.sizeY() - heatLevelY,
                                       guiControl.fillerX(),
            ...
            // TANK GAUGE Y
            // ------------
            if (guiControl.getControlType().equals( GUIControlTypeEnum.TankGaugeY ))
            {
                displayGauge( screenX,
                              screenY,
                              guiControl,
    ...
    {
        super.drawGuiContainerForegroundLayer( mouseX,
                                               mouseY );

        // Name displayed above the GUI, typically name of the furnace.
        String machineNameLocalized = StatCollector.translateToLocal( "tile." +
                                                                      this.ENTITY.getMachineInternalName() +
                                                                      ".name" );
        ...
        this.fontRenderer.drawString( machineNameLocalized,
                                      this.xSize / 2 - this.fontRenderer.getStringWidth( machineNameLocalized ) / 2,
                                      6,
                                      4210752 );

        // Text that labels player inventory area as "Inventory".
        String inventoryWordLocalized = I18n.getString( "container.inventory" );
        this.fontRenderer.drawString( inventoryWordLocalized,
                                      8,
                                      this.ySize - 96 + 2,
                                      4210752 );
        ...
        for (int i = 0; i < this.GUICONTROLS.length; i++)
        {
            ...
            // Power level X OR Y
            if (guiControl.getControlType().equals( GUIControlTypeEnum.PowerLevelX ) ||
                guiControl.getControlType().equals( GUIControlTypeEnum.PowerLevelY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          ...
                                          mouseY ))
                {
                    String powerLevelLiteral = String.valueOf( this.ENTITY.getEnergyStored( ForgeDirection.UNKNOWN ) ) +
                                               "/" +
                                               String.valueOf( this.ENTITY.getMaxEnergyStored( ForgeDirection.UNKNOWN ) );
                    String powerLevelLocalized = StatCollector.translateToLocal( "tooltip.energy" ) + " ";
                    this.drawTooltip( mouseX - this.guiLeft,
                                      mouseY - this.guiTop + 10,
                                      powerLevelLocalized +
                                      String.valueOf( this.ENTITY.getPowerRemainingScaled( 100 ) ) +
                                      " %",
                                      powerLevelLiteral );
                }
            }
            ...
            if (guiControl.getControlType().equals( GUIControlTypeEnum.CookingProgressX ) ||
                guiControl.getControlType().equals( GUIControlTypeEnum.CookingProgressY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          guiControl.screenY(),
                                          guiControl.sizeX(),
                                          ...
                                          mouseY ))
                {
                    String cookingProgressLiteral = String.valueOf( this.ENTITY.getProgressValue() ) +
                                                    "/" +
                                                    String.valueOf( this.ENTITY.getProgressMaximum() );
                    String cookingLevelLocalized = StatCollector.translateToLocal( "tooltip.progress" ) + " ";
                    this.drawTooltip( mouseX - this.guiLeft,
                                      mouseY - this.guiTop + 10,
                                      cookingLevelLocalized +
                                      String.valueOf( this.ENTITY.getItemCookTimeScaled( 100 ) ) +
                                      " %",
                                      cookingProgressLiteral );
                }
            }
            ...
            if (guiControl.getControlType().equals( GUIControlTypeEnum.TankGaugeY ))
            {
                                          ...
                                          mouseY ))
                {
                    if (this.ENTITY.getFluidStack() != null)
                    {
                        this.drawTooltip( mouseX - this.guiLeft,
                                          mouseY - this.guiTop + 10,
                                          this.ENTITY.getFluidLocalizedName(),
                                          this.ENTITY.getFluidAmount() + " L" );
                    }
                }
            }
            ...
            if (guiControl.getControlType().equals( GUIControlTypeEnum.DamageLevelX ) ||
                guiControl.getControlType().equals( GUIControlTypeEnum.DamageLevelY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          guiControl.screenY(),
                                          guiControl.sizeX(),
                                          ...
                                          mouseY ))
                {
                    String dmgLevelLiteral = String.valueOf( this.ENTITY.getDamageValue() ) +
                                             "/" +
                                             String.valueOf( this.ENTITY.getDamageMaximum() );
                    String dmgLevelLocalized = StatCollector.translateToLocal( "tooltip.damage" ) + " ";
                    this.drawTooltip( mouseX - this.guiLeft,
                                      mouseY - this.guiTop + 10,
                                      dmgLevelLocalized +
                                      String.valueOf( this.ENTITY.getDamageValueScaled( 100 ) ) +
                                      " %",
                                      dmgLevelLiteral );
                }
            }
            ...
            if (guiControl.getControlType().equals( GUIControlTypeEnum.HeatGaugeX ) ||
                guiControl.getControlType().equals( GUIControlTypeEnum.HeatGaugeY ))
            {
                if (this.isPointInRegion( guiControl.screenX(),
                                          guiControl.screenY(),
                                          guiControl.sizeX(),
                                          ...
                                          mouseY ))
                {
                    String heatLevelLiteral = String.valueOf( this.ENTITY.getHeatLevelValue() ) +
                                              "/" +
                                              String.valueOf( this.ENTITY.getHeatLevelMaximum() );
                    String heatLevelLocalized = StatCollector.translateToLocal( "tooltip.heat" ) + " ";
                    this.drawTooltip( mouseX - this.guiLeft,
                                      mouseY - this.guiTop + 10,
                                      heatLevelLocalized +
                                      String.valueOf( this.ENTITY.getHeatLevelTimeScaled( 100 ) ) +
                                      " %",
                                      heatLevelLiteral );
                }
            }
        ...
        for (int i = 0; i < CONTAINERS.length; i++)
        {
            ...
            ContainerTemplate slotContainer = CONTAINERS[i];
        ...
        for (int i = 0; i < BUTTONS.length; i++)
        {
            ...
            // Help link.
            if (guiButton.getButtonType().equals( GUIButtonTypeEnum.InvisibleButton ))
            {
                // Check if mouse cursor is within the given region of the GUI.
    ...
    {
        this.drawTextWithTooltip( textName,
                                  format,
                                  x,
                                  y,
                                  mouseX,
    ...
    private void drawTextWithTooltip(String textName, String format, int x, int y, int mouseX, int mouseY, int color)
    {
        ...
        String text = format.replaceAll( "%1",
                                         name );
        ...
        if (! GuiScreen.isShiftKeyDown())
        {
            if (toolTips != null)
            {
                ...
                this.drawGradientRect( var6 - 3,
                                       ...
                                       var10,
                                       var10 );
                this.drawGradientRect( var6 - 4,
                                       var7 - 3,
                                       var6 - 3,
                                       var7 + var9 + 3,
                                       var10,
                                       var10 );
                this.drawGradientRect( var6 + var5 + 3,
                                       var7 - 3,
                                       var6 + var5 + 4,
                                       var7 + var9 + 3,
                                       var10,
                ...
                this.drawGradientRect( var6 - 3,
                                       ...
                                       var11,
                                       var12 );
                this.drawGradientRect( var6 + var5 + 2,
                                       var7 - 3 + 1,
                                       var6 + var5 + 3,
                                       var7 + var9 + 3 - 1,
                                       var11,
                ...
                for (int var13 = 0; var13 < toolTips.length; ++ var13)
                {
                    ...
                    this.fontRenderer.drawStringWithShadow( var14,
                                                            var6,
                                                            var7,
                                                            - 1 );
                    var7 += 10;
    ...
    public void initGui()
    {
        ...
        for (int i = 0; i < BUTTONS.length; i++)
        {
            ...
            if (guiButton.getButtonType().equals( GUIButtonTypeEnum.InvisibleButton ))
            {
                buttonList.add( new GUIButtonInvisibleControl( guiButton.buttonID(),
                                                               posX + 81,
                                                               posY - 76,
                                                               guiButton.sizeX(),
                                                               guiButton.sizeY() ) );
