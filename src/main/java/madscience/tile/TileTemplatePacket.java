...
import madscience.network.PacketList;
...
public class TileTemplatePacket extends PacketList
{
    /**
     ...
     */
    private TileEntityPrefab madTileEntity;
    ...
    public TileTemplatePacket()
    {
        // Required for reflection.
    }

    public TileTemplatePacket(TileEntityPrefab madMachine) // NO_UCD (use default)
    {
        // ------
        // SERVER
        // ------
        ...
        this.lastEnergy = madMachine.getEnergy();
        this.lastEnergyMaximum = madMachine.getMaxEnergy();
        ...
        for (ModelData modelData : madMachine.getEntityModelData())
        {
            ...
            singleCompressesdModelPiece.append( String.valueOf( MiscUtils.convertBooleanToByte( modelData.isModelVisible() ) ) );
        ...
        this.entityModelData = MiscUtils.removeLastChar( compressedPacketModelData.toString() );

        // World render information.
        WorldRenderInfo worldRenderInfo = madMachine.getEntityWorldRenderInformation();
    ...
    public void execute(EntityPlayer player, Side side) throws ProtocolException
    {
        ...
        if (side.isClient())
        {
            ...
            TileEntity possibleTileEntity = player.worldObj.getBlockTileEntity( tilePosX,
                                                                                tilePosY,
                                                                                tilePosZ );
            if (possibleTileEntity instanceof TileEntityPrefab)
            {
                madTileEntity = (TileEntityPrefab) possibleTileEntity;
            }
            ...
            // Energy.
            this.madTileEntity.setEnergy( lastEnergy );
            ...
            for (String splitServerModel : splitServerModelCompressesdData)
            {
                ...
                {
                    throw new IllegalArgumentException( "[" + this.machineName +
                                                        "]Invalid model packet information, cannot update model instance!" );
                }

                String modelName = packetModelComponents[0];
                byte modelVisible = Byte.parseByte( packetModelComponents[1] );

                this.madTileEntity.setModelWorldRenderVisibilityByName( modelName,
                                                                        MiscUtils.convertByteToBoolean( modelVisible ) );
            }
            ...
            {
                throw new IllegalArgumentException( "[" + this.machineName +
                                                    "]Invalid world render information packet, cannot update world render information!" );
            }
            ...
            if (splitWorldRenderPosition[0] == null ||
                splitWorldRenderPosition[1] == null ||
                splitWorldRenderPosition[2] == null)
            {
                throw new IllegalArgumentException( "[" + this.machineName +
                                                    "]Invalid world render position data, cannot update world render information!" );
            }

            // Check if the scale data is valid.
            if (splitWorldRenderScale[0] == null ||
                splitWorldRenderScale[1] == null ||
                splitWorldRenderScale[2] == null)
            {
                throw new IllegalArgumentException( "[" + this.machineName +
                                                    "]Invalid world render scale data, cannot update world render information!" );
            }
            ...
            ModelPosition receivedPosition = new ModelPosition( floatRenderPosition[0],
                                                                floatRenderPosition[1],
                                                                floatRenderPosition[2] );

            // Create scale object from received data.
            ModelScale receivedScale = new ModelScale( floatRenderScale[0],
                                                       floatRenderScale[1],
                                                       floatRenderScale[2] );
            ...
            {
                throw new IllegalArgumentException( "[" + this.machineName +
                                                    "]Invalid world render data, cannot create world render objects from received packet data!" );
            }

            // Apply the data to the client.
            this.madTileEntity.setWorldRenderInformation( receivedPosition,
                                                          receivedScale );
        }
        else
        {
    ...
    public void read(ByteArrayDataInput in) throws ProtocolException
    {
        ...
        this.lastEnergyMaximum = in.readInt();
    ...
    public void write(ByteArrayDataOutput out)
    {
        ...
        // Stores last known amount of energy this block was known to have.
        out.writeInt( this.lastEnergy );
        out.writeInt( this.lastEnergyMaximum );
