...
public class TileEntityHeatPrefab extends TileEntityEnergyPrefab
{
    /**
    ...
    public TileEntityHeatPrefab()
    {
        super();
    }

    TileEntityHeatPrefab(TileEntityFactoryProduct registeredMachine)
    {
        super( registeredMachine );

        // Apply information from factory object if we have any.
        HeatLevelsSupported[] heatSupported = registeredMachine.getHeatLevelsSupported();
        ...
        if (heatSupported != null)
        {
            ...
            for (HeatLevelsSupported heatSupport : heatSupported)
            {
    ...
    public void writeToNBT(NBTTagCompound nbt)
    {
        ...
        if (this.heatLevelSupported)
        {
            ...
            nbt.setBoolean( "HeatSupported",
                            this.heatLevelSupported );

            // Amount of heat current stored within the block.
            nbt.setInteger( "HeatValue",
                            this.heatLevelValue );

            // Trigger value where we will return true for isHeated.
            nbt.setInteger( "HeatTrigger",
                            this.heatLevelTriggerValue );

            // Total heat level possible within this block.
            nbt.setInteger( "HeatMaximum",
                            this.heatLevelMaximum );
        }
    }
