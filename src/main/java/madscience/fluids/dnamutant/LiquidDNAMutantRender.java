...
public class LiquidDNAMutantRender implements ISimpleBlockRenderingHandler
{
    @Override
    public int getRenderId()
    {
        // Returns the hooked render ID from mod initialization.
        return MadFluids.liquidDNAMutantRenderID;
    }

    @ForgeSubscribe
    public void postStitch(TextureStitchEvent.Post event) // NO_UCD (unused code)
    {
        MadFluids.LIQUIDDNA_MUTANT.setIcons(MadFluids.LIQUIDDNA_MUTANT_BLOCK.getBlockTextureFromSide(0), MadFluids.LIQUIDDNA_MUTANT_BLOCK.getBlockTextureFromSide(1));
    }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
    {
        ...
        if (block.getRenderType() != MadFluids.liquidDNAMutantRenderID)
        {
            return true;
        }
