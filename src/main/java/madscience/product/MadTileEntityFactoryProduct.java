...
import madscience.block.MadGhostBlockData;
...
import madscience.damage.MadDamage;
import madscience.data.MadTileEntityFactoryProductData;
import madscience.energy.MadEnergy;
import madscience.fluid.MadFluid;
import madscience.heat.MadHeat;
...
import madscience.tile.MadTileTemplateBlock;
import madscience.tile.MadTileTemplateContainer;
import madscience.tile.MadTileTemplateGUI;
...
public class MadTileEntityFactoryProduct
{
    ...
    {
        return new MadTileTemplateGUI( playerEntity,
                                       worldEntity );
    }
    ...
    {
        return new MadTileTemplateContainer( playerEntity,
                                             worldEntity );
    }
    ...
    public void loadMachineInternalRecipes()
    {
        ...
        {
            MadModLoader.log().warning( "[MadTileEntityFactoryProduct]Unable to load internal machine recipes for '" +
        ...
        for (MadRecipe machineRecipe : this.data.getRecipeArchive())
        {
            ...
            for (MadRecipeComponent inputIngredient : machineRecipe.getInputIngredientsArray())
            {
                if (inputIngredient.getSlotType().name().toLowerCase().contains( "input" ))
                {
                    ...
                    ItemStack inputItem = MadRecipe.getItemStackFromString( inputIngredient.getModID(),
                                                                            inputIngredient.getInternalName(),
                ...
                {
                    MadModLoader.log().info( "[" + this.data.getMachineName() +
                                             "]Bad Input: " +
            ...
            for (MadRecipeComponent outputResult : machineRecipe.getOutputResultsArray())
            {
                if (outputResult.getSlotType().name().toLowerCase().contains( "output" ))
                {
                    ...
                    ItemStack outputStack = MadRecipe.getItemStackFromString( outputResult.getModID(),
                                                                              outputResult.getInternalName(),
    ...
    public String[] loadSoundArchive()
    {
        ...
        for (int i = 0; i < data.getSoundArchive().length; i++)
        {
                ...
                machineSound.getSoundRandomVariance() > 0)
            {
                ...
                {
                    MadModLoader.log().info( "[" + this.getMachineName() +
    ...
    {
        MadModelBounds defaultBoundingBox = new MadModelBounds( new MadModelPosition( 0.0F,
                                                                                      0.0F,
                                                                                      0.0F ),
                                                                new MadModelPosition( 1.0F,
                                                                                      1.0F,
                                                                                      1.0F ) );
    ...
    public void setMultiBlockDefaults()
    {
        ...
        MadGhostBlockData machineGhostBlocks = new MadGhostBlockData( 0,
                                                                      0,
                                                                      0 );
