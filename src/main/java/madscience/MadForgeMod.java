...
public class MadForgeMod
{
    ...
    public void preInit(FMLPreInitializationEvent event) // NO_UCD (unused code)
    {
        ...
        // Registers all blocks with factory.
        this.loadBlocks();
    ...
    private void recipeBlocks()
    {
        ...
        for (Iterator iterator = registeredBlocks.iterator(); iterator.hasNext();)
        {
            ...
            {                
                // Recipes for crafting the block.
                registeredBlock.loadCraftingRecipes();
                
                // Recipes for cooking the block into something else in vanilla Furnace.
                registeredBlock.loadVanillaFurnaceRecipes();
            }
        }
    }
    ...
    private void configureBlocks(Configuration config)
    {
        MadBlockFactoryProductData[] blockData = MadMod.getUnregisteredBlocks();
        for (int x = 0; x < blockData.length; x++) 
        {
            MadBlockFactoryProductData unregisteredBlock = blockData[x];
            ...
            unregisteredBlock.setBlockID(defaultBlockID);
            
            // Get configuration file information, if there is any...
            int configBlockID = config.getBlock(Configuration.CATEGORY_BLOCK, unregisteredBlock.getBlockName(), unregisteredBlock.getBlockID()).getInt();
            
            // Check if unregistered machine default ID is different from read value.
            if (unregisteredBlock.getBlockID() == configBlockID)
            {
                MadMod.log().info("[" + unregisteredBlock.getBlockName() + "]Using default block ID of " + String.valueOf(configBlockID));
            }
            else
            {
                MadMod.log().info("[" + unregisteredBlock.getBlockName() + "]Using user configured block ID of " + String.valueOf(configBlockID));
            }
        }
    }
    ...
    private void loadBlocks()
    {
        ...
        MadBlockFactoryProductData[] machineData = MadMod.getUnregisteredBlocks();
        for (int i = 0; i < machineData.length; i++) 
        {
            MadBlockFactoryProductData unregisteredMachine = machineData[i];
            MadBlockFactory.instance().registerBlock(unregisteredMachine);
        }
    }
